...
for _ in range(10000):
    ...
    base_car_price = random.randint(15000, 50000)
    
     # Introduce variability within each car model
    model_price_variation = random.uniform(-5000, 5000)
    ...
    fuel_type = np.random.choice(["Gasoline", "Diesel", "Electric", "Hybrid"], p=[0.25, 0.1, 0.5, 0.15])
...
print("CSV file 'car_data.csv' has been generated with 10,000 rows of data.")
=======
import pandas as pd
import random
...
# Generate 10,000 rows of data
for _ in range(10000):
    car_make, car_model = random.choice(car_makes_and_models)
    base_car_price = random.randint(15000, 50000)
    ...
    car_price = round(base_car_price + model_price_variation)
    car_year = 2023
    car_type = random.choice(["Sedan", "SUV", "Truck", "Coupe"])
    car_color = random.choice(["Red", "Blue", "Black", "White", "Silver", "Gray"])
    number_of_doors = random.choice([3, 5])
    ...
    # Scale the weights to cover the desired price range
    scaled_weights = [w / total_weight for w in weights]
    fuel_type = random.choices(["Gasoline", "Diesel", "Electric", "Hybrid"], weights=scaled_weights)[0]
